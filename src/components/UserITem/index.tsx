...
const UserItem: FC<{ user: User; index: number }> = ({ user, index }) => {
    const dispatch = useAppDispatch();

    const dragStartHandler = (i: number, e: DragEvent<HTMLLIElement>) => {
        dispatch(setCurrentCard(i));
    };
    ...
    const dropHandler = (index: number, e: DragEvent<HTMLLIElement>) => {
        e.preventDefault();
        const element = e.target as HTMLLIElement;
        dispatch(userListFilter(index));
        if (element.id === "block") {
            element.style.background =
                "rgb(249 250 251 / var(--tw-bg-opacity))";
        }
                ...
                >
                    ...
                    <div className="btn_block flex justify-center gap-2 min-w-10 items-center">
                        <span
                            ...
                            onClick={() => {
                                dispatch(selectUser(user.index));
                                dispatch(toggleChangeUser(true));
